...
for i in np.arange(2,len(temp)):
    if temp[" Fnlj(N)"][i-1]<200 and temp[" Fnlj(N)"][i]>200:
...
### Straightening of force curves to eliminate deviation
    ...
    # correction is an array containing, for each brake, mean values of Fn and Ft, before and after braking
    # d is a list containing, for a given brake, mean values of Fn and Ft, before and after braking
    # Mean values are aclculated before braking between -5s and -1s, and after braking between 1s and 5s to avoid noise
...
for i in range(len(data)):
    ...
    df=pd.DataFrame()
    mask=(temp["t(s)"]>data['debut'][i]-5) & (temp["t(s)"]<data['debut'][i]-1)
    df=temp[mask]
    ftmoy_av=df[' Ftlj(N)'].mean()
    fnmoy_av=df[' Fnlj(N)'].mean()

    df=pd.DataFrame()
    mask=(temp["t(s)"]<data['fin'][i]+5) & (temp["t(s)"]>data['fin'][i]+1)
    df=temp[mask]
    ftmoy_ap=df[' Ftlj(N)'].mean()
    fnmoy_ap=df[' Fnlj(N)'].mean()

    d={'fn_av':fnmoy_av,'ft_av':ftmoy_av,'fn_ap':fnmoy_ap,'ft_ap':ftmoy_ap}
    
    correction=correction.append(d,ignore_index=True)

   # For each brake, force curves are straightened by subtracting linear deviations over its duration
...
### Plot of µ, Fn and Ft for each brake
    ...
    plt.plot(df["t(s)"],df["µ"], color='rebeccapurple', label=f'µ{i+1}') # darkcyan, tab:blue, rebeccapurple
    #plt.grid()
    plt.ylim(0.25,0.55)
    #plt.xlim(-0.5,26.5)
    ...
    Colors_Patin = ['lightcoral','red','darkred','orange','yellow','lime','limegreen','darkgreen','cyan','blue','violet','darkviolet']
    Colors_Disque = ['rosybrown','chartreuse','lemonchiffon','deeppink','lavender','magenta','hotpink']
...
### Plot of temperatures for each brake
    ...
    for column in df_pad.columns :
        ...
        plt.grid()  
    plt.xlabel('Temps de freinage (s)')
    plt.ylim(0, 150)
    plt.ylabel('Temperatures patins (°C)')
    plt.title(Titre + f', Brake {i+1}')
    ...
    for column in df_disque.columns :
        num = ''
        for k in column:
            if k!='T' and k!='d' and k!='i' and k!='s' and k!='q' and k!='u' and k!='e':
                num = num + k
        num_ = int(num)
        plt.plot(df["t(s)"],df_disque[column],Colors_Disque[num_],label=column)
    plt.xlabel('Temps de freinage (s)')
    plt.ylim(0, 150)
    plt.ylabel('Temperatures disques (°C)')
    plt.title(Titre + f', Brake {i+1}')
    plt.legend(loc="upper left",bbox_to_anchor=(0,1))
    plt.savefig(directory+ f'Plot_Temp/T_Ser_{i+1}.png')
    #plt.close()
    ...
    plt.ylim(0, 600)
    plt.xlabel('Temps de freinage (s)')
    plt.ylabel('Forces tangentielles et normales (N)')
    plt.title(Titre + f', Brake {i+1}')
    plt.legend(loc="upper left",bbox_to_anchor=(0,1))
    plt.savefig(directory+ f'Plot_Temp/T_Ser_{i+1}.png')
    #plt.close()
    
    plt.figure(100,figsize=(19.2,10.8))
    plt.plot(temp["t(s)"],Fn_filtered,'orange',label='Fn_filtered(N)')      # Fn(N)
    plt.plot(temp["t(s)"],temp[" Fnlj(N)"],'grey',label='Fn(N)')          # Ft(N)
    plt.legend(loc="upper left",bbox_to_anchor=(0,1))
    plt.xlabel('temps (s)')
    plt.ylabel('Force (N)')
    plt.title(Titre)
...
plt.plot(temp["t(s)"],temp[" Ftlj(N)"],'grey',label='Ft(N)')          # Ft(N)
plt.legend(loc="upper left",bbox_to_anchor=(0,1))
plt.xlabel('temps (s)')
plt.ylabel('Force (N)')
plt.title(Titre)
plt.savefig(directory+ f'Plot_Donnees/Donnees.png')
plt.show()
...
plt.plot(temp["t(s)"],temp[" lj1_rpm(t/min)"],'-k',label='Vitesse de rotation(rpm)')      # N(rpm)
